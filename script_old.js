...
async function detectQuestionFiles() {
    ...
    // Añadir el conjunto original (LPI 306)
    questionSets['questions.txt'] = {
        name: 'Examen LPI 306 (Original)',
        questions: null
    };
    
    // Lista de archivos JavaScript potenciales a verificar
    const potentialFiles = [
        'questions_devops_part1.js',
        'questions_devops_part2.js', 
        'questions_devops_part3.js',
        'questions_devops_part4.js',
        'questions_devops_part5.js',
        'questions_devops_part6.js',
        'questions_devops_part7.js',
        'questions_devops_part8.js',
        'questions_devops_part9.js',
        'questions_devops_part10.js',
        'questions_aws_part1.js',
        'questions_aws_part2.js',
        'questions_kubernetes_part1.js',
        'questions_kubernetes_part2.js',
        'questions_docker_part1.js',
        'questions_docker_part2.js',
        'questions_linux_part1.js',
        'questions_linux_part2.js',
        'questions_networking_part1.js',
        'questions_networking_part2.js'
    ];
...
// Las preguntas ahora se cargan dinámicamente desde archivos .js
    {
...
let questions = questions_lpi306; // Por defecto usar las preguntas LPI 306
...
function checkWrittenAnswer() {
    ...
    const question = shuffledQuestions[currentQuestionIndex]; // Obtener la pregunta actual
    const isCorrect = input.value.trim().toLowerCase() === question.correct.toLowerCase();
    
    // Destacar el campo de entrada
    if (isCorrect) {
        input.style.borderColor = 'var(--correct-answer-border)';
        input.style.backgroundColor = 'var(--correct-answer-bg)';
        input.style.color = 'var(--correct-answer-text)';
        showModal('¡Correcto!'); // Mostrar mensaje de correcto
        summary[currentQuestionIndex].correct = true; // Marcar la pregunta como correcta en el resumen
    } else {
        input.style.borderColor = 'var(--incorrect-answer-border)';
        input.style.backgroundColor = 'var(--incorrect-answer-bg)';
        input.style.color = 'var(--incorrect-answer-text)';
...
// Función para destacar respuestas en preguntas de opción única
function highlightAnswers(correctIndex, selectedIndex) {
    ...
    buttons.forEach((button, index) => {
        ...
        } else if (buttonIndex === selectedIndex && selectedIndex !== correctIndex) {
...
// Función para destacar respuestas en preguntas de opción múltiple
...
function updateNavigationButtons() {
    ...
    // El botón siguiente siempre está habilitado para permitir navegación libre
...
function goToNextQuestion() {
    ...
    if (currentQuestionIndex >= shuffledQuestions.length) {
        currentQuestionIndex = 0; // Volver al principio si estamos al final
    }
    showQuestion();
    updateSummaryPanel();
}

// Función para enviar la respuesta actual
function submitCurrentAnswer() {
    const question = shuffledQuestions[currentQuestionIndex];
    
    if (question.type === "written" || question.type === "escribir") {
        checkWrittenAnswer();
...
// Función para cargar un conjunto de preguntas específico
async function loadQuestionSet(fileName) {
    ...
    try {
        if (fileName === 'questions.txt') {
            // Usar las preguntas originales que ya están cargadas
            questions = questions_lpi306;
            showModal('Conjunto de preguntas LPI 306 cargado correctamente.');
        } else {
            // Cargar dinámicamente el archivo JavaScript
            const script = document.createElement('script');
            script.src = fileName;
            script.onload = () => {
                // Determinar qué variable de preguntas usar basándose en el nombre del archivo
                if (fileName.includes('part1')) {
                    questions = questions_devops_part1;
                } else if (fileName.includes('part2')) {
                    questions = questions_devops_part2;
                } else if (fileName.includes('part3')) {
                    questions = questions_devops_part3;
                } else if (fileName.includes('part4')) {
                    questions = questions_devops_part4;
                }
                
                showModal(`Conjunto de preguntas cargado correctamente: ${questionSets[fileName].name}`);
                initializeQuiz();
                document.head.removeChild(script); // Limpiar el script del DOM
            };
            script.onerror = () => {
                showModal('Error al cargar el conjunto de preguntas. Usando el conjunto por defecto.');
                questions = questions_lpi306;
                initializeQuiz();
    ...
    loadButton.textContent = 'Cargar Preguntas';
}

// Event listener para el botón de cargar preguntas
